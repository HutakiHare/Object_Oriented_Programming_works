...
class IoT_ctrl_payload : public payload {
    ...
    public:
        ...
        SET(set_parent,unsigned int, parent,now_parent);
        GET(get_parent_counter,int, parent_counter);
        SET(set_parent_counter,int,parent_counter,new_parent_counter);
...
class AGG_ctrl_payload : public payload {
        ...
        unsigned int my_packet_num;
        unsigned int my_parent;    // send to sink, tell dst parent 
        string my_relation;
    ...
    public:
        ...
        set<unsigned int> my_same_counter_parent;
        // get & set my_parent
        GET(get_parent,unsigned int,my_parent);
        SET(set_parent,unsigned int, my_parent, new_parent);
        GET(get_mypacket,unsigned int,my_packet_num);
        SET(set_mypacket,unsigned int, my_packet_num, new_packet);
        GET(get_my_relation,string,my_relation);
        SET(set_my_relation,string, my_relation, new_relation);
        GET(get_mycounter,unsigned int, mycounter);
        SET(set_mycounter,unsigned int, mycounter, new_counter);
...
class DIS_ctrl_payload : public payload {
        ...
        // unsigned int counter ;
        unsigned int parent;  // the parent dst node to change to
    ...
    public:
        ...
        // set & get the parent for dst node to change to
        SET(setParent,unsigned int,parent,new_parent);
...
class IoT_device: public node {
        ...
        bool hi = false;     //  record first packet -> directly store as info 
        int counter=0;       //  #hop
        unsigned int pkt_size=0;  // packet size
        unsigned int parent=0;
        vector<pair<unsigned int, bool>> children;
        map<unsigned int, unsigned int> same_counter_parent;
    ...
    public:
        ...
        /***********************************************/
        GET(get_pkt_size,unsigned int, pkt_size);    // get packet size
        GET(get_parent, unsigned int, parent);       //get parent
        GET(get_counter,int,counter);                // get counter
        SET(set_counter,int,counter,precounter);     // set counter to precounter
        SET(set_pkt_size,unsigned int, pkt_size,new_pkt_size);  //set packet size
        map<unsigned int, unsigned int> reverse_path;   // record reverse path (dstID, nextID)
        /************************************************/
...
class IoT_sink: public node {
        ...
        bool hi = false;     // record of first packet, directly store as info
        int counter=0;       // #hop
        unsigned int pkt_max_size=0; //record allowed packet size
        unsigned int parent=0;
        vector<pair<unsigned int, bool>> children;
    ...
    public:
        ...
        GET(get_pkt_size,unsigned int, pkt_max_size);     // get packet size
        GET(get_parent, unsigned int, parent);       //get parent
        GET(get_counter,int,counter);                // get counter
        SET(set_counter,int,counter,precounter);     // set counter to precounter
        SET(set_pkt_size,unsigned int, pkt_max_size,new_pkt_size);  // set packet size
        map<unsigned int, unsigned int> reverse_path;  // map of reverse path (dstID, nextID)
        map<unsigned int, unsigned int> all_node_parent;  // record the ID send up from AGG (nodeID, nodeID's parent)
        map<unsigned int, unsigned int> all_node_packet_size;
        map<unsigned int, set<unsigned int> > all_node_same_counter_parent;
        set<unsigned int> leaves;
        map<unsigned int, unsigned int> all_node_counter; 
...
void IoT_device::recv_handler (packet *p){
    ...
    if (p->type() == "IoT_ctrl_packet" ) { // the device receives a packet from the sink
        // dynamic cast packet to IoT_ctrl_packet
        ...
        // if is packet is from children, go in here, no packet send out
        if(hi&&(l3->getCounter()>= this->get_counter())){   // children  (counter > now)

            // parent is changed ? -> check whether the changed-to-parent has smaller counter than now / smaller id (when counter same)
            if((l3->get_parent()!=getNodeID())&&((l3->get_parent_counter()<this->get_counter())||((l3->get_parent_counter()==this->get_counter())&&l3->get_parent()<getNodeID()))){ 
                // search through chilren to remove it from vecter
                for (auto it = this->children.begin(); it != this->children.end(); it ++) {    
                    if((p3->getHeader()->getPreID()==it->first)){   //found
        ...
        // if is packet is from parent, go in here, send packet if being modified
        if (( l3->getCounter()< this->get_counter()|| !hi)) {   // parents  (counter < now)
            
            l3->increase();    // to now need add one to counter

            // remove it from children list if it's inside
            for (auto it = this->children.begin(); (it != this->children.end()&&hi); it ++) {
                if(it->first == p3->getHeader()->getPreID()){
                    children.erase(it);
                    break;
                }
            }
            /**************************************************************************************************************************/
            if(!same_counter_parent.empty()||!hi){
                if(hi&&l3->getCounter()-1<same_counter_parent.begin()->second){
                    same_counter_parent.clear();
                    same_counter_parent.insert({p3->getHeader()->getPreID(),l3->getCounter()-1});
                }else{
                    same_counter_parent.insert({p3->getHeader()->getPreID(),l3->getCounter()-1});
                }
            }
            ...
            // node need to update 
            if((p3->getHeader()->getPreID() < this->parent||( l3->getCounter()<this->get_counter())) ||!hi){

                this->parent = p3->getHeader()->getPreID();  //change parent
                this->set_counter(l3->getCounter());         // set counter
                
                // only enter when receiving first packet -> all neighbor except parent -> children
                if(!hi){
                    ...
                    for (map<unsigned int,bool>::const_iterator it = nblist.begin(); it != nblist.end(); it ++) {
                        if(it->first!=p->getHeader()->getPreID()){
                            this->children.push_back(make_pair(it->first,false));
                        }
                    }
                }
                ...
                l3->set_parent(this->parent);                   // store node's parent in packet
                l3->set_parent_counter(this->get_counter()-1);  // store parent's counter
                p3->getHeader()->setPreID ( getNodeID() );      // set pre ID
                p3->getHeader()->setNexID ( BROADCAST_ID );     // send to all neighbors
                p3->getHeader()->setDstID ( BROADCAST_ID );
                send_handler(p3);                               // send packet
            }
        }
        hi = true;       //has been entered
    }
    else if (p->type() == "IoT_data_packet" ) {   // the device receives a packet
        //cout << "node " << getNodeID() << " send the packet" << endl;

        // dynamic cast packet to IoT_data_packet
        ...
        bool send_packet=true;        // see if received all packet from child

        // receive packet from child, make false to true 
        for (auto it = this->children.begin(); it != this->children.end(); it ++) {
            if(it->first==p3->getHeader()->getPreID()){
                it->second=true;
                break;
            }else if(it+1==children.end()){
               // cout<<getNodeID()<<" children "<<p->getHeader()->getPreID()<<" not found\n";
                break;
            }
        }

        // see if there is any children not yet receive
        for (auto it = this->children.begin(); it != this->children.end(); it ++) {
            if(it->second == false){
                send_packet=false;
                break;
        ...
        p3->getHeader()->setNexID ( this->parent );
        p3->getHeader()->setDstID ( 0 );
        if(send_packet&&getNodeID()!=0){   // send packet if all children get & nowID not 0
            send_handler(p3);              // send packet
        }
        ...
        // reverse path if is neighbor -> dstID == nextID
        if(p3->getHeader()->getPreID()!=this->getNodeID()){
            reverse_path.insert({(p3->getHeader()->getPreID()),p3->getHeader()->getPreID()});
        }

        // stringstream to extract msg 
        stringstream ss;
        ss.str("");
        string message = l3->getMsg(); // msg format -> num space num ...
        ss << message;
        unsigned int dstID;             // store what dstID go pass current node
        while(ss>>dstID&&(ss.str()!="default")){  //if still got info
            this->reverse_path.insert({dstID,p3->getHeader()->getPreID()});  //insert to now-node's reverse path (dstID, PreID)
        }

        string Msg_to_send = "";  //initialize Msg to send

        // formed msg, put all node go pass cur node to sink into msg
        for(map<unsigned int, unsigned int>::iterator it=this->reverse_path.begin();it!=reverse_path.end();it++){
            Msg_to_send+=to_string(it->first)+" ";
        }

        //if no node go pass -> default
        if(Msg_to_send.empty()){
            Msg_to_send="default";
        }

        // if the sending node == source -> record parent for DIS to use
        if(p3->getHeader()->getSrcID()==getNodeID()){
            l3->set_parent(this->parent);
            for(map<unsigned int, unsigned int>::iterator it = same_counter_parent.begin(); it != same_counter_parent.end(); it++){
                l3->my_same_counter_parent.insert(it->first);
            }
            l3->set_mypacket(pkt_size);
            if(children.empty()){
                l3->set_my_relation("leaf");
            }else{
                l3->set_my_relation("parent");
            }
            l3->set_mycounter(get_counter());
        }

        // update packet
        l3->setMsg(Msg_to_send);  //msg to send into payload
        //p3->getHeader()->setSrcID ( getNodeID() );
        p3->getHeader()->setPreID ( getNodeID() );  //PreID = curID
        p3->getHeader()->setNexID ( this->parent );  // NextId = parent
        p3->getHeader()->setDstID ( 0 );             // dstID -sink
        send_handler(p3);   // send packet
        ...
        // get packet from sink (source)  ** keep most info unchange ex: dstID's new parent, sourceID, dstID
        if(p3->getHeader()->getDstID()==getNodeID()){  //if now node is dst
            this->parent = l3 -> getParent();          // change parent 
        }else{                                         // if not yet finish sending 
            p3->getHeader()->setPreID ( getNodeID() ); // PreID = now
            p3->getHeader()->setNexID ( reverse_path[p3->getHeader()->getDstID()] ); // know nextID by reverse path list
            send_handler(p3);                           //send packet
...
void IoT_sink::recv_handler (packet* p){
    ...
    if (p->type() == "IoT_ctrl_packet" ) { // the device receives a packet from the sink
        ...
        // reverse path if is neighbor -> dstID == nextID
        if(p3->getHeader()->getPreID()!=this->getNodeID()){
            this->reverse_path.insert(pair<unsigned int, unsigned int>(p3->getHeader()->getPreID(),p3->getHeader()->getPreID()));
        }

        // insert all node's parent to map
        all_node_parent.insert({p3->getHeader()->getSrcID(),l3->get_parent()});

        /*********************************************************************************************************/
        all_node_packet_size.insert({p3->getHeader()->getSrcID(),l3->get_mypacket()});
        all_node_counter.insert({p3->getHeader()->getSrcID(), l3->get_mycounter()});
        
        //cout<<"***********"<<p3->getHeader()->getSrcID()<<": ";
        for(set<unsigned int>::iterator it = l3->my_same_counter_parent.begin(); it != l3->my_same_counter_parent.end(); it++){
            all_node_same_counter_parent[p3->getHeader()->getSrcID()].insert(*it);
            //cout<<*it<<" "; 
        }
        //cout<<"\n";

        if(l3->get_my_relation() == "leaf"){
            leaves.insert(p3->getHeader()->getSrcID());
        }

        /**********************************************************************************************************/

        //extract msg by stringstream
        stringstream ss;
        ss.str("");
        ...
        unsigned int dstID;
        while(ss>>dstID){
            this->reverse_path.insert({dstID,p3->getHeader()->getPreID()});  // (dstID, nextID) -> nextID == node who send here
        }
    }
    else if (p->type() == "DIS_ctrl_packet") {
        DIS_ctrl_packet *p3 = nullptr;
...
/****************************************************************************************************************
        ...
        for(map<unsigned int, unsigned int>:: iterator it = all_node_counter.begin(); it !=all_node_counter.end(); it ++){
            if(it->second > max_counter){
                max_counter = it->second;
            }
        }
        //cout<<max_counter<<"\n";
        for(unsigned int level = max_counter; level > 0 ; level--){
            for(map<unsigned int, unsigned int>:: iterator it1 = all_node_counter.begin(); it1 != all_node_counter.end(); it1 ++){
                if(it1->second != level){
                    continue;
                }
                //cout<<"now level: "<<level<<" in "<<it1->first<<"\n";
                unsigned int now_computation;
                for(set<unsigned int>::iterator parents = all_node_same_counter_parent[it1->first].begin(); parents != all_node_same_counter_parent[it1->first].end() ; parents++){
                    //cout<< it1->first<<"+"<<*parents<<" = "<< all_node_packet_size[it1->first]+all_node_packet_size[*parents]<<" , "<< (all_node_packet_size[it1->first]+all_node_packet_size[*parents])%pkt_max_size<<"\n";
                    now_computation=(all_node_packet_size[it1->first]+all_node_packet_size[*parents])%pkt_max_size;
                    if(now_computation==0){
                        //cout<<*parents<<"'s in to "<< it1->first<<"!!!\n";
                        all_node_parent[it1->first] = *parents ;
                        //all_node_packet_size[*parents]+=all_node_packet_size[it1->first];
                        break;
                    }
                }
            }
...
/******************************************************************************************************************************************/
        ...
        for(set<unsigned int>::iterator it1 = leaves.begin(); it1 != leaves.end(); it1++){
            unsigned int now_computation;
            for(set<unsigned int>::iterator parents = all_node_same_counter_parent[*it1].begin(); parents != all_node_same_counter_parent[*it1].end() ; parents++){
                now_computation=(all_node_packet_size[*it1]+all_node_packet_size[*parents])%pkt_max_size;
                if(now_computation==0){
                    all_node_parent[*it1] = *parents;
                    all_node_packet_size[*parents]+=all_node_packet_size[*it1];
                    break;
                }
    ...
    /*    
        unsigned int sum_pkt = INT_MAX, sum_data = 0;
        stack<unsigned int> BFS_record, min_route;
        ...
        for(set<unsigned int>::iterator it1 = leaves.begin(); it1 != leaves.end(); it1++){
            ...
            while(!BFS_record.empty()){
                sum_data+=all_node_packet_size[BFS_record.top()];
                if(all_node_parent[BFS_record.top()]==0||(all_node_same_counter_parent[BFS_record.top()].find(min_route.top())!=all_node_same_counter_parent[BFS_record.top()].end())){
                    if((sum_data/pkt_max_size)<sum_pkt){
                        while(!min_route.empty()){
                            min_route.pop();
                        }
                        sum_pkt=(sum_data/pkt_max_size);
                        min_route.push(BFS_record.top());
                        BFS_record.pop();
                    }else if(all_node_same_counter_parent[BFS_record.top()].find(min_route.top())!=all_node_same_counter_parent[BFS_record.top()].end()){
                        all_node_parent[BFS_record.top()]=min_route.top();
                ...
                for(set<unsigned int>::iterator parents = all_node_same_counter_parent[BFS_record.top()].begin(); parents != all_node_same_counter_parent[BFS_record.top()].end() ; parents++){
                    BFS_record.push(*parents);
                }
            }
        }
*/
        ...
        this->parent=0;                                         // sink parent initialize
        p3->getHeader()->setSrcID ( 0 );                        // source = sink
        p3->getHeader()->setPreID ( getNodeID() );              // PreID = sink
        for(map<unsigned int,unsigned int>::iterator it=reverse_path.begin(); it!=reverse_path.end(); it++){  // send to all nodes
            l3->setParent(all_node_parent[it->first]);   // know new_parent of dstID by all node parent list 
            p3->getHeader()->setNexID ( it->second );    // nextID = reverse_path[dstID]
            p3->getHeader()->setDstID ( it->first );     // dstID
            send_handler(p3);                            // send packet
        }
...
int main(){
    ...
    unsigned int node_num,link_num,sim_time,bfs_strt_time,data_trans_time;
    unsigned int pkt_size,agg_strt_time,dis_strt_time;
    cin>>node_num>>link_num>>pkt_size;
    cin>>sim_time>>bfs_strt_time>>agg_strt_time;
    cin>>dis_strt_time>>data_trans_time;

    node::node_generator::generate("IoT_sink",0);     //  generate sink ( 0 )

    for (unsigned int id = 1; id < node_num; id ++){  // generate nodes
        node::node_generator::generate("IoT_device",id);
    }
   
    unsigned int node_id, data_sz;
    IoT_sink* sink = dynamic_cast<IoT_sink*>(node::id_to_node(0));
    cin>>node_id>>data_sz;
    sink->set_pkt_size(pkt_size);
    // dynamic cast node to IoT_device, then print out
    for(int i = 1; i < node_num; i++){
        cin>>node_id>>data_sz;
        IoT_device* node = dynamic_cast<IoT_device*>(node::id_to_node(node_id));
        node->set_pkt_size(data_sz);
    }

    unsigned int link_id,link_end1,link_end2;
    // set devices' neighbors
    for (unsigned int lk = 0; lk < link_num; lk ++ ){
    ...
    // node 0 broadcasts a msg with counter 0 at time bfs_strt_time
    IoT_ctrl_packet_event(0,bfs_strt_time);
    // 1st parameter: the source; the destination that want to broadcast a msg with counter 0 (i.e., match ID)
    // 2nd parameter: time (optional)
    // 3rd parameter: msg for debug information (optional)
    
    // all node sends a packet to node 0 at time data_trans_time --> need to implement routing tables for IoT_devicees
    for(int i=1;i<node_num;i++){
        IoT_data_packet_event(i, 0, data_trans_time); // IoT_data_packet
    }
    ...
    for(int i=1; i<node_num; i++){
        AGG_ctrl_packet_event(i, 0, agg_strt_time);  // AGG packet (from leaves to root)
    }
    // 1st parameter: the source node
    // 2nd parameter: the destination node (sink)
    // 3rd parameter: time (optional)
    ...
    DIS_ctrl_packet_event(0, dis_strt_time);        // DIS packet (from root to leaves)
    ...
    // dynamic cast node to IoT_sink
    sink = dynamic_cast<IoT_sink*>(node::id_to_node(0));
    cout<<"0 "<<sink->get_parent()<<"\n";  // print out root's parent

    // dynamic cast node to IoT_device, then print out
    for(int i = 1; i < node_num; i++){
        IoT_device* node = dynamic_cast<IoT_device*>(node::id_to_node(i));
        cout<<i<<" "<<node->get_parent()<<"\n";
    }
